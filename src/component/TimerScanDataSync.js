...
class TimerScanDataSync extends React.PureComponent {
    ...
    startSync(token) {
        //当同步开始执行时，设置
        if (TimerScanDataSync.scanpartsyncing == 0) {
            TimerScanDataSync.scanpartsyncing = 1;
        } else {
            return;
        }
    ...
    //重新开始同步部件扫描
    restartSync(outtime) {
        TimerScanDataSync.scanpartsyncing = 0; //处理完成
    ...
    startSync2(token) {
        //当同步开始执行时，设置
        if (TimerScanDataSync.boxphotosyncing == 0) {
            TimerScanDataSync.boxphotosyncing = 1;
        } else {
            return;
        }
    ...
    restartSync2(outtime) {
        TimerScanDataSync.boxphotosyncing = 0; //处理完成
    ...
    //重置执行器
    resetSync() {
        TimerScanDataSync.scanpartsyncing = 0;
        TimerScanDataSync.boxphotosyncing = 0;
        this.timer && clearTimeout(this.timer);
        this.timer2 && clearTimeout(this.timer2);
        this.timer = null;
        this.timer2 = null;
