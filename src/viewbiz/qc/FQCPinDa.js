...
class FQCPinDa extends React.Component {
        ...
        HTTPPOST('/sm/updateBoxHG', data, token)
            .then((res) => {
                if (res.code >= 1) {
                    ...
                    Toast.info('提交拼搭检验结果成功！', 1, undefined, false);
    ...
    getboxes() {
        ...
        if (theorderno == "") {
            ...
            Toast.info('查询【' + theorderno + '】待检验箱子...', 1, undefined, false);
        }
            ...
            .then((res) => {
                if (res.code >= 1) {
                    if (res.list && res.list.length >= 1) {
                        ...
                        Toast.info('合同【' + theorderno + '】待检验箱子，接收成功！', 1, undefined, false);
                        this.setState({ searchloading: false });
                    } else {
                        this.setState({ fqcboxes: [] });
                        this.setState({ otherboxes: [] });

                        Toast.info('合同【' + theorderno + '】无待检验箱，请知悉！', 1, undefined, false);
                        this.setState({ searchloading: false });
                    }
    ...
    addboxClick() {
        if (this.state.boxselected) {
            ...
            Toast.info('箱子【' + obj1.boxName + '】添加检验任务成功！', 1, undefined, false);
        } else {
        ...
        await HTTPPOST_Multipart('/sm/uploadPDSMPhotoParam', formdata, token)
            .then((res) => {
                if (res.code > 0) {
                    ...
                    Toast.info('上传照片【' + imgpath + '】成功！', 1, undefined, false);
                } else {
