...
import { WhiteSpace, WingBlank, Flex, List, Switch, Provider, Toast } from '@ant-design/react-native';
...
class FQCPinDaHandle extends React.Component {
    constructor(props) {
        super(props);
        ...
        HTTPPOST('/sm/updateBoxUnHG', data, token)
            .then((res) => {
                if (res.code >= 1) {
                    ...
                    Toast.info('提交不合格处理结果成功！', 1, undefined, false);
    ...
    getboxes() {
        ...
        } else if (this.state.orderno.indexOf(' ') >= 0) {
            theorderno = this.state.orderno.substring(0, this.state.orderno.indexOf(' '));
        }
        else {
            ...
            .then((res) => {
                if (res.code >= 1) {
                    if (res.list && res.list.length) {
                        ...
                        Toast.info('合同【' + theorderno + '】拼搭不合格箱子，接收成功！', 1, undefined, false);
                        this.setState({ searchloading: false });
                    } else {

                        Toast.info('合同【' + theorderno + '】，无不合格箱！', 1, undefined, false);
                        this.setState({ searchloading: false });
                    }
